* **Instructions**
  ...
  * In your server.js file, you will create four routes: `get`, `post`, `put`, and `delete`.
    ...
    * Additionally, show the form that the user can use to add a movie to be watched.  When the submit button is clicked, the code will post to the `'/api/movies'` route, which will insert the movie from the form into the movies table and return the ID of the new movie.
