// This application uses two different NPM packages to provide
// the complete address and weather information about any location.
// Ex: "node weatherdest Paris, France" or "node weatherdest Tokyo, Japan"

// Mainly it combines the code from the last two activities with a
// bit more data handling. (Study it on your own time if interested).
...
geocoder.geocode(address, function(err, data) {
  ...
  if (gAddress.city) {
    search += gAddress.city;
  }

  if (gAddress.stateCode) {
    search += ', ' + gAddress.stateCode;
  }

  if (gAddress.zipcode) {
    search += ' ' + gAddress.zipcode;
  }

  if (gAddress.countryCode) {
    search += ' ' + gAddress.countryCode;
  }
  ...
  weather.find({search: search, degreeType: 'F'}, function(err, result) {
    ...
    if (err) {
      ...
      console.log(`Sorry we don't have enough data on that location!
      Try somewhere else.`);
